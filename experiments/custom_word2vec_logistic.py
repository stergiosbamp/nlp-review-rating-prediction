...
from embeddings import CustomWord2VecVectorizer
...
if __name__ == '__main__':
    ...
    doc_vectorizer = CustomWord2VecVectorizer(
        size=100, model_path="../data/software_custom.word2vec")
    text_clf = TextClassification(doc_vectorizer,
                                  LogisticRegression(solver='saga', C=1.5))
    text_clf.fit(X_train_res, y_train_res)
    y_predicted = text_clf.predict(X_test)
    text_clf.classification_report(
        y_test,
        y_predicted,
        filename="../findings/WORD2VEC/custom_word2vec-100.csv",
        save=True)
    print("MEA: {:.5f}".format(text_clf.mean_absolute_error(
        y_test, y_predicted)))
